...
import { 
  MapPin, Check, Info, FileText, Calendar, Compass, 
  Phone, MessageSquare, Mail, Layers, Landmark, Download, X
} from "lucide-react";
    ...
    <div className="w-full bg-dark-bg relative overflow-hidden pb-16">
          ...
          <div className="max-w-4xl flex flex-col space-y-4 items-start">
            ...
            <div className="flex flex-wrap items-center gap-y-2 gap-x-4 text-xs font-sans text-warm-muted">
              <span className="flex items-center space-x-1.5 text-gold">
                ...
                <span className="tracking-wide uppercase font-bold">{project.location}</span>
              </span>
              <span className="hidden sm:inline">|</span>
              <span>HARERA Reg: <strong className="text-warm-white">{project.rera.replace(/\[\[|\]\]/g, "")}</strong></span>
            </div>
      ...
      <section className="max-w-7xl mx-auto px-6 md:px-12 pt-12 relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
          ...
          {/* Left Column (8/12): Tabs, Floorplans, Map */}
          <div className="lg:col-span-8 flex flex-col space-y-10">
            ...
            <div className="flex border-b border-gold-border/20">
              {["overview", "specs", "location"].map((tab) => (
                <button
                  ...
                  className={`px-6 py-4.5 text-[10px] font-sans uppercase tracking-widest border-b-2 font-bold transition-all ${
                    activeTab === tab
            ...
            {activeTab === "overview" && (
              <div className="space-y-8 animate-fadeIn">
                <div className="space-y-4">
                  <h3 className="font-serif text-2xl font-bold text-warm-white">Project Overview</h3>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-6 bg-dark-surface/40 border border-gold-border/20 p-6 md:p-8">
                  <div className="flex flex-col space-y-1">
                    <span className="text-[10px] text-warm-muted uppercase tracking-widest">Starting Price</span>
                    <span className="font-serif text-lg font-bold text-gold">{project.startingPrice.replace(/\[\[|\]\]/g, "")}</span>
                  </div>
                  <div className="flex flex-col space-y-1">
                    <span className="text-[10px] text-warm-muted uppercase tracking-widest">Configurations</span>
                    <span className="font-serif text-lg font-bold text-gold">{project.configurations.replace(/\[\[|\]\]/g, "")}</span>
                  </div>
                  <div className="flex flex-col space-y-1">
                    <span className="text-[10px] text-warm-muted uppercase tracking-widest">Unit Sizes</span>
                    <span className="font-serif text-lg font-bold text-gold">{project.unitSizes.replace(/\[\[|\]\]/g, "")}</span>
                  </div>
                  <div className="flex flex-col space-y-1 sm:col-span-1 pt-4 border-t border-gold-border/10 sm:border-none sm:pt-0">
                    <span className="text-[10px] text-warm-muted uppercase tracking-widest">Possession Date</span>
                    <span className="font-serif text-lg font-bold text-gold">{project.possessionDate.replace(/\[\[|\]\]/g, "")}</span>
                  </div>
                  <div className="flex flex-col space-y-1 pt-4 border-t border-gold-border/10 sm:border-none sm:pt-0">
                    <span className="text-[10px] text-warm-muted uppercase tracking-widest">Project Type</span>
                    <span className="font-serif text-lg font-bold text-gold">{project.type}</span>
                  </div>
                  <div className="flex flex-col space-y-1 pt-4 border-t border-gold-border/10 sm:border-none sm:pt-0">
                    <span className="text-[10px] text-warm-muted uppercase tracking-widest">License Status</span>
                    <span className="font-serif text-lg font-bold text-gold">RERA Approved</span>
                  </div>
                </div>
                ...
                {project.highlights.length > 0 && (
                  <div className="space-y-4">
                    <h4 className="font-serif text-lg font-bold text-warm-white">Key Highlights</h4>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
                      {project.highlights.map((h, i) => (
                        <div key={i} className="flex items-start space-x-2 font-sans text-xs">
                          <Check className="w-4 h-4 text-gold shrink-0 mt-0.5" />
                          <div>
                            <strong className="text-warm-white block">{h.label}</strong>
                            <span className="text-warm-muted font-light">{h.value}</span>
                          </div>
                        </div>
                      ))}
                    </div>
            ...
            {/* Tab 2: Specifications Panel */}
            {activeTab === "specs" && (
              <div className="space-y-8 animate-fadeIn">
                <h3 className="font-serif text-2xl font-bold text-warm-white">Architectural Specifications</h3>
                
                {project.specifications.length === 0 ? (
                  <p className="text-xs text-warm-muted uppercase tracking-widest">Specifications will be updated shortly.</p>
                ) : (
                  <div className="space-y-6">
                    {project.specifications.map((spec, idx) => (
                      <div key={idx} className="border-b border-gold-border/10 pb-5 last:border-none">
            ...
            {/* Tab 3: Location Advantages Panel */}
            {activeTab === "location" && (
              <div className="space-y-8 animate-fadeIn">
                <h3 className="font-serif text-2xl font-bold text-warm-white">Connectivity & Locations</h3>
                
                {project.locationAdvantages.length === 0 ? (
                  <p className="text-xs text-warm-muted uppercase tracking-widest">Location advantages will be updated shortly.</p>
                ) : (
                ...
                {project.images.masterPlan && project.images.masterPlan.startsWith("/") && (
                  <div className="mt-8 border border-gold-border/20 p-6 bg-dark-surface/40 flex flex-col space-y-4">
                    <h4 className="font-serif text-base font-bold text-warm-white">Location Map & Proximity</h4>
                    ...
                    <div className="flex flex-col sm:flex-row justify-between items-center gap-4 text-xs pt-2 font-sans">
                      <span className="text-warm-muted">Sector-7 Sohna, Gurugram Location Layout Map</span>
                      <a
            ...
            {/* 3. Clearly Demarcated Floorplan Placeholders or Active Floor Plan Viewer */}
            <div className="border-t border-gold-border/25 pt-10 space-y-6">
              <h3 className="font-serif text-2xl font-bold text-warm-white">Floor & Master Plans</h3>
              
              {project.images.floorPlans && project.images.floorPlans.length > 0 && project.images.floorPlans[0].startsWith("/") ? (
                <div className="space-y-6">
                  {/* Selectors grid */}
                  <div className="flex flex-wrap gap-2">
                    {project.images.floorPlans.map((plan, idx) => {
                      const filename = plan.split('/').pop() || "";
                      let displayName = filename.replace(/-/g, " ").replace(/\.[^/.]+$/, "");
                      if (filename.includes("161")) displayName = "161 Stilt Floor";
                      else if (filename.includes("162-168-stilt")) displayName = "162-168 Stilt Floor";
                      else if (filename.includes("162-168-typical")) displayName = "162-168 Typical Floor";
                      else if (filename.includes("162-168-terrrace")) displayName = "162-168 Terrace";
                      else if (filename.includes("171-177-typical-stilt")) displayName = "171-177 Typical-Stilt Floor";
                      else if (filename.includes("171-177-typical")) displayName = "171-177 Typical Floor";
                  ...
                  <div className="border border-gold-border/20 bg-dark-surface/40 p-6 flex flex-col items-center space-y-4">
                    ...
                    >
                      <Image
                        src={project.images.floorPlans[selectedFloorPlan]}
                        alt={project.images.floorPlans[selectedFloorPlan].split('/').pop() || "Floor Plan Layout"}
                        fill
                    ...
                    {/* Preview controls and download link */}
                    <div className="w-full flex flex-col sm:flex-row justify-between items-center gap-4 text-xs pt-2 font-sans">
                      <div className="text-warm-muted text-center sm:text-left">
                        <span className="font-bold text-warm-white uppercase tracking-wider block text-[10px]">Active Blueprint:</span>
                        {(() => {
                          const plan = project.images.floorPlans[selectedFloorPlan];
                          const filename = plan.split('/').pop() || "";
                          if (filename.includes("161")) return "161 Stilt Floor Plan";
                          if (filename.includes("162-168-stilt")) return "162-168 Stilt Floor Plan";
                          if (filename.includes("162-168-typical")) return "162-168 Typical Floor Plan";
                          if (filename.includes("162-168-terrrace")) return "162-168 Terrace Floor Plan";
                          if (filename.includes("171-177-typical-stilt")) return "171-177 Typical-Stilt Floor Plan";
                          if (filename.includes("171-177-typical")) return "171-177 Typical Floor Plan";
                          return filename;
                        })()}
                      </div>
                      <div className="flex space-x-3 w-full sm:w-auto justify-center">
                        ...
                        <a 
                          href={project.images.floorPlans[selectedFloorPlan]} 
                          download
            ...
            <div className="border-t border-gold-border/25 pt-10 space-y-6">
              ...
              <div className="border border-gold-border/20 bg-dark-surface/20 p-6 space-y-4">
                ...
                <div className="space-y-3 font-sans text-xs text-warm-muted font-light pl-6 border-l border-gold-border/20">
                  <div>
                    <strong className="text-warm-white block uppercase tracking-wider text-[10px]">Q1 2026 update:</strong>
                    <p>{project.images.constructionUpdates[0]?.replace(/\[\[|\]\]/g, "") || "RCC Foundation slabs and pillar frames casting completed."}</p>
                  </div>
                  <div className="pt-2">
                    <strong className="text-warm-white block uppercase tracking-wider text-[10px]">Q2 2026 update:</strong>
                    <p>{project.images.constructionUpdates[1]?.replace(/\[\[|\]\]/g, "") || "Internal partition brickwork and electrical conduit mappings ongoing."}</p>
                  </div>
                </div>
              </div>
            </div>
          ...
          {/* Right Column (4/12): Sticky Enquiry Sidebar */}
          <div className="lg:col-span-4">
            <aside className="sticky top-[100px] bg-dark-surface border border-gold-border p-6 md:p-8 space-y-6 shadow-2xl">
              
              <div className="space-y-1">
                <h4 className="font-serif text-xl font-bold text-warm-white">Brochure & Price Sheet</h4>
                <p className="text-[10px] uppercase tracking-widest text-gold font-bold">Project Downloads</p>
              </div>
        ...
        <section className="max-w-7xl mx-auto px-6 md:px-12 pt-20 border-t border-gold-border/15 mt-20 relative z-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {relatedProjects.map((p) => (
              <div key={p.id} className="border border-gold-border/20 bg-dark-surface p-6 flex flex-col justify-between space-y-6 group">
                <div className="space-y-2">
                  <span className="text-[9px] uppercase tracking-widest text-gold font-bold">{p.type}</span>
                  <h4 className="font-serif text-xl font-bold text-warm-white group-hover:text-gold transition-colors">{p.name}</h4>
                ...
                <Link href={`/projects/${p.id}`} className="text-xs font-bold text-gold uppercase tracking-widest hover:underline inline-flex items-center space-x-1">
      ...
      {isFloorPlanLightboxOpen && project.images.floorPlans && project.images.floorPlans.length > 0 && (
        <div 
        ...
        >
          ...
          >
            <span className="font-sans text-xs uppercase tracking-widest font-bold flex items-center gap-1">Close X</span>
          </button>
