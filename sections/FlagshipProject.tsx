...
import { MapPin, ArrowRight, ShieldCheck, CornerDownRight } from "lucide-react";
...
export function FlagshipProject() {
  ...
  return (
    <section className="relative py-20 md:py-28 bg-dark-bg border-t border-gold-border/10 overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12 relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 items-center">
          ...
          <div className="lg:col-span-5 flex flex-col space-y-6 items-start">
            <div className="flex flex-col space-y-2">
              ...
              <div className="flex items-center space-x-1.5 text-gold text-xs font-sans mt-1">
                ...
                <span className="tracking-wide uppercase">{project.location}</span>
              </div>
            </div>
            ...
            <div className="flex flex-wrap gap-2 pt-2">
              {project.features.map((feature, idx) => (
                <span 
                  key={idx} 
                  className="px-3.5 py-1.5 border border-gold-border/25 bg-dark-surface/40 backdrop-blur-md rounded-none text-[10px] uppercase tracking-wider text-warm-white"
                >
                  {feature}
                </span>
              ))}
          ...
          <div className="lg:col-span-7 flex flex-col space-y-4">
            <div className="relative overflow-hidden border border-gold-border/30 bg-dark-surface shadow-2xl">
              {/* Embla Viewport */}
              <div ref={emblaRef} className="overflow-hidden cursor-grab active:cursor-grabbing">
                <div className="flex">
                  {project.images.gallery.map((src, index) => (
                    <div key={index} className="flex-[0_0_100%] min-w-0 relative h-[280px] md:h-[400px] w-full">
                      <Image
                        src={src}
                        alt={`Slide ${index + 1}`}
                        fill
              ...
              {/* Limited Units Badge */}
              <div className="absolute top-4 right-4 bg-gold text-dark-bg text-[9px] uppercase tracking-[0.25em] font-bold px-3 py-1.5 z-10 shadow-lg">
                Ongoing · Limited Units
              </div>

              {/* HARERA badge watermark */}
              <div className="absolute bottom-4 left-4 text-[9px] uppercase tracking-widest text-warm-white/70 font-sans z-10">
                ...
                {scrollSnaps.map((_, index) => (
                  <button
                    ...
                    className={`w-1.5 h-1.5 rounded-full transition-all duration-300 ${
                      index === selectedIndex ? "bg-gold w-6" : "bg-gold-border/40 hover:bg-gold/40"
                    }`}
